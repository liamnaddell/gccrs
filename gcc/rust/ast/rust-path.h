...
protected:
  tl::optional<std::vector<PathExprSegment>> segments;
...
public:
  ...
  bool is_lang_item () const
  {
    rust_debug ("[ARTHUR] lang_item: %s",
		lang_item.has_value () ? "yes" : "no");
    rust_debug ("[ARTHUR] ident_segment: %s",
		ident_segment.has_value () ? "yes" : "no");
    rust_debug ("[ARTHUR] %s", as_string ().c_str ());

    return lang_item.has_value ();
  }
